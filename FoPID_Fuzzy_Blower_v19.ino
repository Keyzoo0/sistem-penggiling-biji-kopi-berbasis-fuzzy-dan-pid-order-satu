// =============================================================================
//  FoPID + Fuzzy Type-1 BLOWER Control — v19
//
//  ╔══════════════════════════════════════════════════════════════════════════╗
...
//
//  DIPERTAHANKAN dari v17:
//  [SERVO-MANUAL] STATE_SERVO_PREFUZZY, NVS, servo langsung gerak
//  [COUNTDOWN] Timer logging countdown MM:SS dengan blink <5 menit
//  [MON-1..7] RPM monitoring 25–37 RPM, quadrature encoder, rpmDryer/8
//  [FIX-LCD-REC] Indikator [REC] berkedip
//  [FIX-RPM-INIT] Buffer RPM langsung stabil dengan inisialisasi buffer
//  [FIX-AUTORECORD] Auto record saat masuk MODE_FUZZY
//
//  FILE YANG PERLU ADA:
//    1. FoPID_Fuzzy_Blower_v19.ino ← file ini
//    2. Fis_Header_v19.h           ← GANTI dari Fis_Header.h lama
//       (rename jadi "Fis_Header.h" agar #include "Fis_Header.h" tetap jalan)
// =============================================================================
...
#include "Fis_Header.h"   // → gunakan Fis_Header_v19.h, rename jadi Fis_Header.h

// ─── Variabel Fuzzy & FoPID ───────────────────────────────────────────────────
// [V19-1] Error utama tetap dalam PERSEN untuk FoPID (sesuai gambar referensi)
float errorPersen      = 0.0f;
float prevErrorPersen  = 0.0f;
float deltaErrorPersen = 0.0f;

// Error dalam °C — dipakai sebagai INPUT FIS langsung (bukan %)
float errorFuzzy       = 0.0f;   // = SP - suhuActual (°C) → fisIn1
float deltaErrorFuzzy  = 0.0f;   // = errorFuzzy - prevErrorFuzzy (°C) → basis fisIn2
float prevErrorFuzzy   = 0.0f;

float setPointSuhu     = 60.0f;
// [V19-5] Inisialisasi lebih aman: 45% (bukan 60%)
float outputFuzzy      = 45.0f;

// [V19-3] Parameter FoPID dikonservatifkan
float betaBlower = 0.40f;   // 0.55 → 0.40
float Kp     = 0.60f;       // 1.80 → 0.60
float Ki     = 0.08f;       // 0.15 → 0.08
float Kd     = 0.50f;       // 0.90 → 0.50
float lambda = 0.90f;       // tetap
float mu     = 0.92f;       // tetap
...
//  DIMMER_MAX dinaikkan ke 85 karena FIS output max 75% + FoPID 6% = 81%
//  Memberi ruang ~4% lebih untuk keamanan.
#define DIMMER_MIN 20
#define DIMMER_MAX 85   // [V19-4] naik dari 80 → 85
...
// [V19-5] Inisialisasi ke 50 (bukan DIMMER_MAX) → transisi lebih halus
int dimmerOut       = 50;
...
void startLogging() {
  ...
  logFile.print("# FoPID+Fuzzy Blower v19 | Start:");
  ...
  logFile.print("# [V19] FIS_in=errorFuzzy(C) | FIS_out=[30..75]% | ");
...
void monitorView() {
  ...
  switch (page) {
    ...
    case 2:
      // Debug: tampilkan error°C dan errorPersen
      lcd.setCursor(0, 0);
...
//
//  ╔══════════════════════════════════════════════════════════════════════════╗
//  ║  PERBAIKAN UTAMA: INPUT FIS LANGSUNG DALAM °C, PARAMETER KONSERVATIF    ║
//  ╚══════════════════════════════════════════════════════════════════════════╝
//
//  [V19-1] INPUT FIS DALAM °C LANGSUNG:
//    fisIn1 = constrain(errorFuzzy, -10, 40)  ← error°C (SP - suhuActual)
//    Tidak ada mapFloat berlapis — lebih bersih dan mudah di-debug.
//    Range FIS v19 dikalibrasi ke [-10..40]°C sesuai kondisi nyata percobaan.
//
//  [V19-2] DELTA ERROR FIS DALAM °C → dipetakan ke [0..5]:
//    fisIn2 = clamp((deltaErrorFuzzy + 5) / 10 * 5, 0, 5)
//    Tengah range: deltaErr=0 → fisIn2=2.5 (MF 'sedang' paling aktif)
//    Suhu naik cepat: deltaErr negatif → fisIn2 kecil → MF 'kecil'
//    Suhu turun/stabil: deltaErr positif → fisIn2 besar → MF 'besar'
//
//  [V19-3] FOPID TETAP BERBASIS errorPersen (%):
//    errorPersen = (SP - suhu) / SP × 100%  ← sesuai gambar referensi
//    Ini HANYA untuk u_fopid, bukan untuk input FIS.
//
//  [V19-4] PARAMETER FoPID KONSERVATIF:
//    Kp=0.60: saat error=35°C, errPct=58.3% → Kp×errPct=35 → tetap saturasi
//             tapi saturasi di ±15 lebih kecil dari ±20 di v17
//             → koreksi FoPID 15×0.40=6% (bukan 20×0.55=11%)
//    Efek total saat suhu 25°C: FIS=45% + 6% = 51% dimmer
//
//  [V19-5] ANTI-WINDUP & DECAY:
//    integral clip: ±40 (dari ±50)
//    decay: 0.96x saat |errorFuzzy| < 2°C (lebih agresif dari 0.98x di v17)
//    Threshold dalam °C (bukan %) — lebih langsung dan akurat
// =============================================================================
void taskFuzzyFoPID() {
  if (runMode != MODE_FUZZY) return;

  // ─── Hitung error dan delta error ──────────────────────────────────────
  // Error dalam °C (untuk FIS dan display)
  errorFuzzy      = setPointSuhu - suhuActual;
  deltaErrorFuzzy = errorFuzzy - prevErrorFuzzy;
  prevErrorFuzzy  = errorFuzzy;

  // Error dalam % (untuk FoPID sesuai gambar referensi)
  ...
  // ─── FoPID fraksional (basis errorPersen) ──────────────────────────────
  integral += powf(DT_FIXED, lambda) * errorPersen;
  // [V19-3] integral clip ±40 (dari ±50)
  integral  = constrain(integral, -40.0f, 40.0f);

  // [V19-5] Decay lebih agresif: 0.96x saat dekat SP (|error| < 2°C)
  ...
  // [V19-3] clip ±15 (dari ±20)
  u_fopid = constrain(u_fopid, -15.0f, 15.0f);

  // ─── Input FIS dalam °C langsung [V19-1] ─────────────────────────────
  // fisIn1: error°C constrain ke range MF v19 [-10..40]
  float fisIn1 = constrain(errorFuzzy, -10.0f, 40.0f);

  // fisIn2: delta error°C → petakan ke [0..5]
  // Tengah (delta=0) → 2.5 (MF 'sedang')
  // Range ±5°C/sampel sudah mencakup kondisi nyata (sampel 500ms)
  ...
  g_fisOutput[0] = 0.0f;
  fis_evaluate();
  outputFuzzy = g_fisOutput[0];  // range [30..75]% sesuai Fis_Header_v19.h

  // ─── Koreksi FoPID + output dimmer final ──────────────────────────────
  // u_fopid > 0: suhu di bawah SP → tambah dimmer → lebih banyak aliran udara panas
  // u_fopid < 0: suhu di atas SP  → kurangi dimmer → blower lebih pelan
...
void setup() {
  Serial.begin(115200);
  Serial.println("=== FoPID+Fuzzy Blower v19 ===");
  Serial.println("[v19] FIS_in=errC | FIS_out=[30..75]% | Kp=0.6 Ki=0.08 Kd=0.5");
  ...
  lcd.setCursor(0, 1); lcd.print("FIS_in=errorC       ");
  ...
  // [V19-5] Mulai dengan 50% — tidak agresif di awal
  dimmer.setPower(50);
  ...
  delay(1500);
  lcd.clear();
  lcd.setCursor(0, 0); lcd.print("v19: Ready          ");
  ...
  Serial.println("[SETUP] v19 siap. Pilih FoPID+Fuzzy → set servo → D untuk mulai.");
  delay(2000);
  lcd.clear();
}
...
//  FIN v19
//
//  RINGKASAN SEMUA PERUBAHAN DARI v17+FisHeader_v18:
//
//  ┌─────────────────────────────────────────────────────────────────────────┐
//  │ # │ Masalah v17+v18           │ Solusi v19                             │
//  ├─────────────────────────────────────────────────────────────────────────┤
//  │ 1 │ Input FIS = mapFloat()    │ Input FIS = errorFuzzy (°C) langsung  │
//  │   │ berlapis (errPct→°C)      │ Lebih bersih, tidak ada konversi ganda │
//  │ 2 │ FIS out min=20%, saat     │ FIS out min=30% → sirkulasi udara panas│
//  │   │ suhu 30°C dimmer hanya    │ lebih cukup → suhu bisa naik ke 60°C  │
//  │   │ 28% (terlalu kecil)       │ Saat suhu 30°C: dimmer ~47-51%        │
//  │ 3 │ Kp=1.80 → u_fopid saturasi│ Kp=0.60 → u_fopid terpakai semua    │
//  │   │ instan di detik pertama   │ informasinya, tidak langsung clip      │
//  │ 4 │ beta=0.55, clip ±20       │ beta=0.40, clip ±15 → koreksi 6%      │
//  │   │ → koreksi 11% (berlebih)  │ (proporsional, tidak mendominasi FIS)  │
//  │ 5 │ DIMMER_MAX=80%            │ DIMMER_MAX=85% (sesuai FIS max 75%)   │
//  │ 6 │ integral clip ±50         │ integral clip ±40 (lebih ketat)        │
//  │ 7 │ decay 0.98x saat <1%     │ decay 0.96x saat |err°C|<2°C          │
//  │   │ (terlalu lambat)          │ (lebih responsif saat mendekati SP)    │
//  │ 8 │ outputFuzzy init=60%      │ outputFuzzy init=45% (tidak agresif)  │
//  │   │ dimmerOut init=DIMMER_MAX │ dimmerOut init=50 (transisi halus)     │
//  └─────────────────────────────────────────────────────────────────────────┘
//
//  HASIL SIMULASI YANG DIVERIFIKASI:
//    Suhu 25°C (error=35°C): FIS=45% + FoPID=6% → dimmer ~51%
//    Suhu 40°C (error=20°C): FIS=44% + FoPID=6% → dimmer ~50%
//    Suhu 55°C (error= 5°C): FIS=57% + FoPID=4% → dimmer ~59%
//    Suhu 60°C (error= 0°C): FIS=63% + FoPID~2% → dimmer ~64%
//    Suhu 65°C (error=-5°C): FIS=65% + FoPID=-3%→ dimmer ~64% (stabil)
// =============================================================================
